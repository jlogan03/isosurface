...
impl<S: ScalarSource> HermiteSource for CentralDifference<S> {
    fn sample_normal(&self, p: Vec3) -> Vec3 {
        let vx = self.sample_scalar(p + Vec3::new(self.epsilon, 0.0, 0.0)).0
            - self.sample_scalar(p - Vec3::new(self.epsilon, 0.0, 0.0)).0;

        let vy = self.sample_scalar(p + Vec3::new(0.0, self.epsilon, 0.0)).0
            - self.sample_scalar(p - Vec3::new(0.0, self.epsilon, 0.0)).0;

        let vz = self.sample_scalar(p + Vec3::new(0.0, 0.0, self.epsilon)).0
            - self.sample_scalar(p - Vec3::new(0.0, 0.0, self.epsilon)).0;
